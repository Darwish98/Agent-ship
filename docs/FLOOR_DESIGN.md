...
Since built: an OS notification when a *run* enters **Needs you** (approval, failure, budget) while the window is unfocused, a **Resume run** action on interrupted runs, and **Land all** on the "Ready to land" lane header (2026-09-22, from a user request) - queues every branch and pending-work session currently shown (not restricted to verified ones, matching what the individual Land button already allows), one at a time through the same test → merge → test → land pipeline, using each one's auto-detected test command and conflict-resolution default. A run that YOU stop mid-queue halts the rest; a branch that fails on its own does not. Already-started lands are real engine runs and keep going even if the dialog is closed. The "Fuel" gauge was also renamed to **Usage** (the old name implied a real, literal capacity reading; it is a self-set budget against local token volume, nothing more - see its tooltip).

An **Autopilot switch** was also added to each registered project's row (2026-09-22, from a user request: "should also be a switch in floor tab when selecting project"). It reflects reality rather than a stored setting: on when the project has a live Autopilot run, off otherwise; clicking it while on stops that run. Clicking it while off ensures the project has the Autopilot flow saved, then checks for `PLAN.md` (see §7's Phase 3 addendum in PLATFORM_PLAN.md for the pattern itself): if it exists, opens the normal run confirmation, pre-filled; if not, opens **Set up a plan** first - paste a plan verbatim, or describe an idea and have an agent expand it into one (a real, budgeted, visible run) - and only then opens that same confirmation. The switch never bypasses the confirmation dialog; it only gets a plan into existence first when one is missing.

Not built, in priority order: an OS notification for ad-hoc sessions; keyboard triage (j/k/enter); search across projects; per-session cost for ad-hoc work; replay of a finished run.
