...
Status: **revision 3.5 (2026-09-22): Phase 3 now also has parallel fan-out/join with a judge, and gates parse the test/lint runner they recognise, see §7. A verification pass (§13) found and fixed a broken packaged-app hook, a triple-installed hook on this machine, and an open relay any web page could post fake events to.** Revision 3.4 (2026-09-21, after first use: landing pipeline, open-session state, Commit & land; plus the first slice of Phase 3: resume-after-restart, orphan sweep, OS notification, see §7). Phases 0, 1, 1.5 and 2 are built. A flow can now run end to end: gated build in its own branch, capped at a dollar ceiling, visible live on both the Floor and the editor.** It has been proven against the real `claude` CLI on toy tasks, not yet on real work. The Floor was redesigned from scratch, see [FLOOR_DESIGN.md](FLOOR_DESIGN.md). Section 12 is the honest evaluation (short answer: a credible wedge, not yet a revolution), and section 11 says what to do next.
...
### Spike results (2026-09-20, full detail in `planning/spikes/claude-cli.md`)
...
- ✅ Fix blank canvas.
- ✅ CLI spike → [`planning/spikes/claude-cli.md`](spikes/claude-cli.md). Answered 4 of the 5 questions from real probes (see §5). Headless `-p` returns exact cost/tokens and schema-validated output, so the engine is cheaper to build than this plan assumed.
...
**Built so far: resume-after-restart, the orphan sweep, an OS notification, the Merge and Land nodes, and parallel fan-out/join with a judge for best-of-N.**
...
- ✅ **Fan-out / Join (parallel copies), with a judge for "best".** The walker was restructured: an agent step and a gate step are functions over a per-walker context, so the run and each copy use the same code. A Fan-out runs the chain that follows it (agents and command gates, repair loops included) N times, up to 4 at once, each copy in its own branch (`agentship/<run>-<node>-c<copy>-a<attempt>`), with its own session, results (`{{node.result}}` inside a copy means *that copy's* node) and `{{copy}}` / `{{copies}}` variables. A Join then decides: **all** (every copy must pass; every branch is kept; no single result branch), **first** (first pass wins, the rest are stopped and their branches deleted), **quorum** (k pass, the rest stopped, all passing branches kept), **best** (a judge agent is shown each passing copy's diff and the criteria and names one; one passer means no judge and no cost; a failed or invalid judge falls back to the cheapest passing copy and says so on the node; losing branches are deleted). The judge is capped by the Join's dollar limit and counted in the run ceiling. A join that can no longer be satisfied stops the remaining copies at once.
  - **Money under parallelism:** each step *reserves* its cap while it runs, so parallel copies are never promised the same remaining dollars; the ceiling already multiplied the copy chain by the copy count and by (1 + gate retries).
  - **Resume:** a run interrupted inside a section runs the whole section again from its Fan-out (copies' directories and branches are cleared first; spend carries over; attempt numbers keep counting so names never collide). After the Join it continues past it in the winner's working copy.
  - **Shape rules, checked before a run starts:** exactly one chain between a Fan-out and its Join; only agents and command gates inside (no human gates, no merge, no nesting); one Fan-out per Join; quorum no larger than the copies. The shipped **Best-of-N tournament** is now runnable end to end: copies each held to your tests, judge picks, winner merged in a scratch copy, re-tested, landed.
  - Tested: 18 engine tests (concurrency, per-copy isolation and results, in-copy repair, every strategy, judge fallback, early stop, money, user stop, queueing past the limit, resume inside and after a section, the whole tournament landing only the winner's file on `main`) and validation tests. Since done: a real 2-copy tournament ran against the real CLI, real judge included (see §13.1: $0.10, 19s, the judge's pick landed). **Not tested:** more than the 4-copy concurrency at real load.
- ✅ **Gates parse the test/lint runner they recognise (`src/shared/gateParse.ts`).** Vitest, Jest, Mocha, pytest, `go test`, `cargo test`, ESLint's stylish output, `tsc`, and a generic JUnit/Surefire-style "Tests run: N, Failures: M" line - all detected from the same combined stdout+stderr tail every gate already keeps, no reporter config required of the user. Gives a `{ tool, summary, passed, failed, total, failures[] }` (each failure a name plus its message, capped at 12). Two things use it, both pure functions of the gate's already-stored output so a resumed run derives the identical result rather than needing a new field on the event: **the repair prompt** now lists the specific failing tests/rules instead of a raw output tail (falls back to the old raw-tail behaviour untouched when the tool or its output format isn't recognised - never worse than before); **the run drawer** shows a compact "2 failed, 12 passed" line on the step and the failure list above the raw output when expanded. Tested: 20 parser tests (one realistic sample per tool, ANSI colour stripped, a "nothing recognised" fallback, the cap) plus two integration tests (the engine's repair prompt, and resume deriving the same feedback from the stored event).
- ✅ **A loop primitive: gates can be checked by an agent (2026-09-22, from a user request for an "autopilot" mode).** A third `check` kind alongside `command` and `human`. Instead of a shell exit code, an agent is asked a yes/no question (structured JSON, `{done, reason}`) and its own `fail` edge is "not done yet, go around again" - reusing the gate's EXISTING retry cap as the loop's iteration cap, and its EXISTING repair-edge mechanism as the loop-back, rather than adding any new engine construct. A missing or malformed answer is never trusted as "done" (fails safe). The call is read-only (`access: 'read'`, though its own `agentTools` can add e.g. `Bash(git log *)`), fresh-session each time (no resume - each judgment should read the CURRENT repo state, not a stale earlier one), and counted in the run's dollar ceiling exactly like a judge or a merge-conflict resolver already are. **Shipped as a new pattern, "Autopilot":** build the next unfinished item from a plan file → test → merge → land → ask "is the plan done?" → loop back to build if not. Landing happens after every item, not once at the end (each pass is a normal, independently-gated Pipeline-shaped step); the plan itself is a repo-relative file path input, which the builder and the checking agent each read for themselves (no new templating - `Read`/`Glob`/`Grep` are already always available to every agent call). The confirmation dialog states the loop's cap and per-call spending limit before a run starts. Drive-by fix found while testing this: `maxRetries: 0` (a real, meaningful "try exactly once, no loop" choice) was wrongly rejected as "no retry cap at all" by validation; the runtime already handled it correctly, only the Problems-panel check was wrong. 11 new tests (loop-then-pass, retry cap exceeded, a malformed answer never treated as done, the check's own budget limit, the run ceiling stopping it before the call is even made, cancellation, and one resume test - an interrupted check is treated exactly like a killed command gate: a failed attempt, repaired before being asked again). **Not tested against the real CLI** (no live test was run for this feature, unlike fan-out/join's).
- ✅ **A Floor switch for Autopilot (2026-09-22, same-day follow-up request).** Each registered project's row on the Floor now has an Autopilot toggle. It has no stored on/off state of its own - "on" means there is a live Autopilot run for that project, checked against the same `runs` the rest of the Floor already reads; clicking it while on cancels that run. Clicking it while off: ensures the project has the Autopilot flow saved (silently, `null`-guarded so a user-edited copy is never overwritten), then checks for a plan file (`PLAN_FILE`, `PLAN.md` by default). If one exists, it goes straight to the normal run confirmation dialog, pre-filled (`RunTarget` gained an optional `initialInputs`, still editable, still requires pressing Start - the switch never bypasses that confirmation). If not, a new **Set up a plan** dialog opens first, with two ways to get one: paste text verbatim (saved as-is, no agent involved), or describe an idea for a new one-agent flow (`buildPlanBlueprint`, `src/main/plan.ts` for the file check/save) to expand into a detailed, numbered, independently-landable plan and write it to the repo - a normal capped ($1) run, visible on the Floor like any other, writing directly to the live checkout (the same choice Supervisor already makes, not a new risk category). Only once a plan exists does it hand off to the same confirmation dialog. 7 new tests (`plan.test.ts`'s file check/save, `buildPlanBlueprint`'s shape). **Not tested:** the Floor toggle and the new dialog themselves - this codebase has no renderer component tests for anything, consistent with how every other Floor feature this session was verified (typecheck + build + engine/shared unit tests only).
  - **Follow-up (2026-09-22, same day):** `docs/` renamed to `planning/` throughout (this file's own folder) - a more accurate name than the generic "docs", since this is the project's own planning record, not user-facing documentation. On a genuinely new project (no existing planning docs when the idea-expansion path runs), `buildPlanBlueprint`'s single agent call now writes a small two-tier package instead of a flat `PLAN.md` alone: `planning/OVERVIEW.md` (the fuller *why* - goals, a phased roadmap) and `planning/DESIGN.md` (the core mechanism's design), generically named rather than reusing this project's own `PLATFORM_PLAN`/`FLOOR_DESIGN` names (which would be a poor fit for, say, a CLI tool or a mobile app). `PLAN.md` at the root stays the same distilled, actionable list, now explicitly told to link back to the fuller docs - the same shape this file and `PLAN.md` itself already have. If `planning/` already holds real documents when the flow runs, the agent is told to read and summarise them into `PLAN.md` rather than overwrite or invent a new idea. One call, no new node kind; budget raised from $1 to $2 to cover the extra writing. `PLAN.md` for this project was written by hand (see the commit that added it) before this richer path existed, so agent-ship itself was not regenerated through it - re-run against a copy is the honest way to prove it end to end, not done here (no live test; costs real money, per the standing "no more e2e" instruction this session). Tests extended, not added: the existing `buildPlanBlueprint` shape test now also checks for both new files and the "do not overwrite" instruction.
- **Still to build in this phase:** parallel branches other than a single copied chain (different agents in parallel, i.e. a real DAG), a multi-branch Merge (merging all the branches an `all` join keeps), and per-copy diversity beyond `{{copy}}` (e.g. different models per copy).
...
3. **Phase 3, in this order:** ~~resume-after-restart and an orphan-worktree sweep~~ (done, see §7); ~~then fan-out/join with a real judge for "best of N"~~ (done, see §7; a real 2-copy tournament has since run against the real CLI, §13.1); ~~parsed test/lint results from gates~~ (done, see §7). Next: a real DAG (parallel branches that are not copies of one chain) and a multi-branch Merge.
...
9. **The Usage gauge (2026-09-22, from a user report that it "seemed buggy") over-counted by ~2.9x on this machine.** Claude Code writes one "assistant" transcript line per *content block* of a response (a thinking block, a text block, each tool call), and every one of those lines repeats the SAME usage object for that whole API call, since usage belongs to the response, not to one block of it. `weeklyUsage()` was summing every line, so one real call with N content blocks was counted N times - confirmed against real local transcripts, where a single tool-heavy response appeared 5 times. Measured on this machine's actual history before/after the fix: 13.8M tokens shown → 4.7M real. Fixed by deduplicating on `message.id` (stable across every line of one call, changes for the next; a line missing it - not observed, but never trusted - falls back to counting itself once rather than being silently dropped). 9 new tests, including one that reproduces the exact multi-block-same-`message.id`-same-usage shape and one confirming the fix actually changes the number (fails as 105 vs. an expected 35 with the dedup removed).
